...
export const getHltbData = async (
    ...
    if (cache && !needCacheUpdate(cache.lastUpdatedAt, cacheHours)) {
        return cache;
    } else {
        ...
        if (result.status === 200) {
            ...
            let gameStats: GameStatsData | undefined = results.data.find(
            ...
            if (gameStats === undefined && results.data.length > 0) {
                const possibleChoices = results.data
                    .map((gameStat) => {
                        return {
                            minEditDistance: get(game, gameStat.game_name, {
                                useCollator: true,
                            }),
                            gameStat,
                        };
...
const useHltb = (appId: number, game: string, serverApi: ServerAPI) => {
    ...
    useEffect(() => {
        if (appId) {
            getHltbData(appId, game, 12, serverApi, stats).then(setStats);
        }
    }, [appId]);
